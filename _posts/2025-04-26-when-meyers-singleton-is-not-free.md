...
Benchmark: [Quick C++ Benchmarks](https://quick-bench.com/q/_FdZTABI5oHQ4a1sHHVtEd15SPE)

Both benchmark functions call `GetInst` and read `v.size()`.

`ManualLifetime` is about 1.5x as fast as `LocalStatic`.
...
Benchmark: [Quick C++ Benchmarks](https://quick-bench.com/q/_S9uabsEzZF7LRExIjh3O4qvGs4)

In both classes, the `std::vector<int>` member is replaced with an `int`. Everything else is the same.

Both benchmark functions perform similarly.
...
A `std::vector<int>` member has a non-trivial destructor, which is why the first benchmark has a guard variable. With an `int` member, neither case applies. `inst` is initialized at compile time and its destructor is trivial, so the first call has no work to do and there is no guard variable. After `GetInst` is inlined, each implementation compiles to a single load from static storage.

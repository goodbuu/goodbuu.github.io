...
The cost difference comes down to when a node is created. Creating a node means allocating memory and constructing a `value_type` object. `BM_EmplaceArgs` hands the key and mapped value to `emplace` as constructor arguments for `value_type`. `emplace` creates the node first, then reads the key from the constructed `value_type` object to do the lookup. If the key is already present, it discards the node.

`BM_InsertPair` hands a const `value_type` object to `insert`. `insert` can access the key directly, so it does the lookup first and creates a node only if the key is absent.

## Avoiding discarded nodes

libc++ uses a key-extraction optimization to avoid creating nodes that would later be discarded. When the type of the first argument is `K` after applying `std::remove_reference_t` and `std::remove_const_t`, `emplace` uses that argument directly for the lookup.

If the key is available on its own, `try_emplace` is the more portable choice: it guarantees that no `value_type` object is constructed if the key is already in the map.
